...
const referrer = cmd_args['referrer'];
const embed_url = cmd_args['embed-url'];

if (!embed_url) {
  console.error('Please provide embed-url');
  process.exit(1);
}

if (!referrer) {
  console.error('Please provide referrer');
  process.exit(1);
}
...
const main = async (embed_url: string, referrer: string) => {
    ...
    let temp_sources = await (await fetch(getSourcesUrl, {
        "headers": {
    ...
    let Q8: any;
    Q8 = temp_sources.t != 0 ? (i(Q5, Q1), Q5) : (Q8 = temp_sources.k, i(Q8, Q1), Q8);

    let str = btoa(String.fromCharCode.apply(null, new Uint8Array(Q8)));
    var decrypted_sources = M(temp_sources.sources, str);
    temp_sources.sources = decrypted_sources;
    
    console.log(JSON.stringify(temp_sources, null, 2));
}

main(embed_url, referrer);
